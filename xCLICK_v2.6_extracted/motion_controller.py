...
class Target:
    ...
    # Optional provider to continuously refresh target coordinates.
    # Useful when targeting tracked UI objects whose position shifts.
    provider: Optional[Callable[[], Tuple[float, float]]] = None
...
class MotionController:
    ...
    def set_target(self, x: float, y: float, label: str = "", confidence: float = 1.0):
        ...
        self.target = Target(x=x, y=y, label=label, confidence=confidence, provider=None)
        if self.state == MotionState.IDLE:
            self.state = MotionState.SEEK
        print(f"  [motion] Target set: ({x:.0f}, {y:.0f}) '{label}' conf={confidence:.2f}")

    def set_target_provider(
        self,
        provider: Callable[[], Tuple[float, float]],
        label: str = "",
        confidence: float = 1.0,
    ):
        """Set a target whose coordinates are refreshed each control tick."""
        x, y = provider()
        self.target = Target(x=float(x), y=float(y), label=label, confidence=confidence, provider=provider)
        if self.state == MotionState.IDLE:
            self.state = MotionState.SEEK
        print(f"  [motion] Target (provider) set: ({x:.0f}, {y:.0f}) '{label}' conf={confidence:.2f}")
    ...
    def _get_target_xy(self) -> Optional[Tuple[float, float]]:
        """Return the freshest target coordinates available."""
        if self.target is None:
            return None
        if self.target.provider is None:
            return (self.target.x, self.target.y)
        try:
            x, y = self.target.provider()
            # Keep last known value on the target too (useful if provider disappears later)
            self.target.x, self.target.y = float(x), float(y)
            return (self.target.x, self.target.y)
        except Exception:
            # Fall back to last stored target coordinates
            return (self.target.x, self.target.y)
    ...
    async def _update(self, dt: float):
        ...
        # Refresh target coordinates from provider (if any)
        if self.target.provider is not None:
            try:
                tx, ty = self.target.provider()
                self.target.x = float(tx)
                self.target.y = float(ty)
            except Exception as e:
                # If provider fails, keep last known coordinates and continue.
                print(f"  [motion] Target provider error: {e}")
    ...
    async def _update_click(self):
        ...
        # Execute click
        await self.click_callback(tx, ty)
        print(f"  [motion] Clicked at ({tx:.0f}, {ty:.0f})")
    ...
    async def seek_and_click_provider(self, provider: Callable[[], Tuple[float, float]], label: str = "") -> bool:
        """Smoothly move to a continuously-updated target and click."""
        self.set_target_provider(provider, label)
    ...
    async def seek_only_provider(self, provider: Callable[[], Tuple[float, float]], label: str = "") -> bool:
        """Smoothly move to a continuously-updated target but don't click."""
        self.set_target_provider(provider, label)
